package com.marketconnect.flume.sink.deletehbase2;
...
import org.apache.flume.sink.hbase2.HBase2Sink;
...
public class DeleteHBase2Sink extends AbstractSink implements Configurable, BatchSizeSupported {
  ...
  public DeleteHBase2Sink() {
    this(HBaseConfiguration.create());
  }

  public DeleteHBase2Sink(Configuration conf) {
    this.config = conf;
  }

  @VisibleForTesting
  @InterfaceAudience.Private
  DeleteHBase2Sink(Configuration conf, DebugIncrementsCallback cb) {
    this(conf);
  ...
  public void configure(Context context) {
    if (!this.hasVersionAtLeast2()) {
      throw new ConfigurationException(
              "HBase major version number must be at least 2 for deletehbase2sink");
    }
    ...
    privilegedExecutor.execute((PrivilegedExceptionAction<Void>) () -> {
      for (Row r : actions) {
        if (r instanceof Delete) {
            ((Delete) r).setDurability(enableWal ? Durability.USE_DEFAULT : Durability.SKIP_WAL);
            Table t = conn.getTable(TableName.valueOf(tableName));
            t.delete((Delete) r);
        } else {
          logger.warn("dropping row " + r + " since it is not a Delete");
        }
      }
      table.flush();
      return null;
